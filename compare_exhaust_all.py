...
position_base = np.load('position_base.npy')
contentV_base = np.load('contentV_base.npy')
contentR_base = np.load('contentR_base.npy')
...
for obs_step in range(obs_num):
    ...
    position = position_base[:, :, obs_step]
    request_arr_V, request_arr_R = env.content_request()
    candidateV, candidateR = env.cal_neighbor()

    content_R = contentR_base[:, obs_step]
    rand_content = contentV_base[:, obs_step]
    content_label = rand_content

    total_reward, packetloss, T_delay, count = EXHAUSTALL(position, candidateV, candidateR, content_label_V,
                                                          content_label_R, rand_content, total_reward, time_delta)
    ...
    for i in range(RSU_num):
        if content_R[i] in content_label_R[i, :]:  # case 1 requested content already in RSU cache
            ...
            for j in range(cache_num_R - 1):
                content_label_R[i][j + 1] = content_label_R[i][j]
            content_label_R[i][0] = content_R[i]
    print('\rObservation Step: {}\t\tCumulative Reward: {:.6f}\t\tRate of Content Received from BS: {:.6f}'.format(
        obs_step, total_reward, BS_count[obs_step]/vehicle_num))
...
print('\rAverage Content Received Rate from BS: {:.6f}'.format(BS_content_rate_average))
